#!/bin/bash
# Overnight WPA2 PSK Cracking Script v2 — WSL2 Kali
# Target: Linksys03963 handshake
# Fix: removed pipe to tail (caused premature continuation), direct output to log
...
echo "=== OVERNIGHT CRACK SESSION v2 (WSL2) ===" > "$LOG"
...
run_attack() {
    ...
    # Run hashcat directly, no pipe — wait for it to finish
    hashcat "$@" --force >> "$LOG" 2>&1
    local rc=$?
